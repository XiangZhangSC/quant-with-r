...
spy_price_change <- calculate_price_change(spy) 
aapl_price_change <- calculate_price_change(aapl)

# make a data frame with daily price change for Coke and Pepsi
price_change_df <- spy_price_change %>% 
  dplyr::bind_rows(aapl_price_change) %>% 
  tidyr::pivot_wider(id_cols = date, names_from = symbol, values_from = diff.xts)
