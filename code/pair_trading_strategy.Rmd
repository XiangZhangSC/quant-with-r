...
# we use the in sample spread beta to contruct the out sample spread
...
ggplot(out_of_sample_spread, aes(date, spread)) + 
  geom_line() + 
  theme_bw() + 
  labs(x = NULL, y = "Spread price (in $)", 
       title = "Out of sample spread between AAPL and SPY")
```

We can visualize both in sample and out of sample spread in a single graph. 
...
in_sample_spread %>% 
  ...
  geom_text(aes(x = lubridate::ymd("2011-07-01"), y = 12), label = "in sample") + 
  ...
  geom_text(aes(x = lubridate::ymd("2012-07-01"), y = 12), label = "out of sample") + 
...
Indeed, as what the texts in the book suggested, we can clearly see that the entire out of sample spread is above the in sample spread. 
...
We can visualize the rolling spread betas and the static spread beta in a single graph. 

```{r rolling-static-beta}
rolling_spread_beta_df %>% 
  dplyr::rename(rolling_beta = spread_beta) %>% 
  dplyr::mutate(static_beta = spread_beta) %>% 
  tidyr::pivot_longer(rolling_beta:static_beta, names_to = "what", values_to = "spread_beta") %>% 
  ggplot(aes(date, spread_beta)) + 
  geom_line(aes(group = what, color = what), linewidth = 1) + 
  scale_color_brewer("", palette = "Set2") + 
  labs(x = NULL, y = "SPread beta") + 
  theme_bw()
```

We can inspect the consequent spread price series when we choose either static or rolling spread beta. 

```{r rolling-static-spread}
in_sample_rolling_spread <- rolling_spread_beta_df %>% 
  dplyr::mutate(spread = purrr::pmap_dbl(list(AAPL, SPY, spread_beta), 
                                     ~calculate_spread(p1 = ..1, 
                                                       p2 = ..2, 
                                                       b = ..3))) %>% 
  dplyr::select(date, spread) %>% 
  dplyr::mutate(what = "rolling")

in_sample_spread %>% 
  dplyr::mutate(what = "static") %>% 
  dplyr::bind_rows(in_sample_rolling_spread) %>% 
  ggplot(aes(date, spread)) + 
  geom_line(aes(group = what, color = what), linewidth = 1) + 
  scale_color_brewer("", palette = "Set2") + 
  theme_bw() + 
